...
def find_closest(location, centroids):
    ...
    close_location = [1000,1000]
    for i in centroids:
        dis = distance(location, i)
        if dis < min_distance:
            min_distance = dis
            close_location = i
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum((xy[0] - mean_x) * (xy[1] - mean_y) for xy in zip(list_of_x_values, list_of_y_values))
    b = Sxy / Sxx
    a = mean_y - b * mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictor_n_r2 = {(find_predictor(user, reviewed, feature_fn))[0]: (find_predictor(user, reviewed, feature_fn))[1]
                      for feature_fn in feature_fns}
    return max(predictor_n_r2, key=predictor_n_r2.get)
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            new_dict[name] = user_rating(user, name)
        else:
            new_dict[name] = predictor(restaurant)
    return new_dict
    # END Question 9
